"""Frozen-v3 sequence-4 generation gate, live-blocked pending fresh preflight."""
...
def verify_exact_generation_attempt(
    *,
    output_root: Path,
    prepared_builder=prepare_frozen_v3_pilot,
    ...
    if provider_fingerprint_builder is None:
        provider_fingerprint_builder = lambda value: make_v3_openai_transport(
    ...
    expected_frozen = {
        ...
        "prompt_digest": "1146474ad5112a238446a63d4fc797022ca2cd65d8e9cb6c88935d7f4f3376e8",
        "schema_version": "moving-service-questions-schema-v3",
        "provider_schema_digest": "333d6923902c46662243e019074b735500904bc49acbafdc1b929bceed9924e2",
        "pilot_configuration_digest": "08d1d6781cae9150c059736ea92e119226234c8e53c798766f2901f010499ad3",
        "provider": "OpenAI",
...
def activate_generation_authority(*, repository_root: Path, output_root: Path,
    ...
    try:
        ...
        if failpoint == "after_authorization_installed":
            raise OSError("synthetic interruption")
        active_manifest = {
            "status": "active_v3_generation_only", "capability": "suggest_moving_service_questions",
...
def _atomic_replace(path: Path, value: bytes) -> None:
    temporary = path.with_name(f".{path.name}.sequence4-v3-generation.tmp")
    descriptor = os.open(
        temporary,
...
def validate_rendered_generation_artifact(artifact: Mapping[str, object], *, now: datetime) -> None:
    ...
    if artifact.get("metadata") != {
        ...
        "authorization_version": "moving-service-openai-v3-generation-sequence-4-v1",
        "authorization_status": "approved_v3_generation", "phase": "generation",
    ...
    if (artifact.get("bindings") != candidate["bindings"]
            or artifact.get("required_v3_preflight") != candidate["required_v3_preflight"]):
...
def verify_unresolved_generation_candidate(
    *, repository_root: Path = REPOSITORY_ROOT,
    ...
    if approved.get("binding_status") != "fresh_v3_preflight_required":
...
def validate_generated_response(raw: object) -> tuple[str, object]:
    ...
    if not isinstance(raw, Mapping):
        return "structural_failure", None
    try:
        v3_response = MovingServiceQuestionResponseV3.model_validate(raw)
    except Exception:
        return "structural_failure", None
    try:
...
def write_generation_outcome(*, output_root: Path, raw: object, now: datetime) -> Mapping[str, object]:
    ...
    if classification == "validated":
        assert isinstance(result, MovingServiceQuestionResponseV3)
