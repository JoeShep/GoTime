...
network_args=""
[ "${GOTIME_V2_SEQUENCE_3_OFFLINE_TEST:-}" = "1" ] && network_args="--network none"
...
exec docker run --rm $network_args --user "$(id -u):$(id -g)" --read-only \
  ...
  --env GOTIME_V2_SEQUENCE_3_OFFLINE_TEST \
  gotime-moving-service-stage-b:openai-2.45.0 sh \
  scripts/experiments/suggest_moving_service_questions/run_openai_stage_b_v2_sequence_3_preflight_container.sh
