"""Sequence-4 candidate, inventory, and same-shell credential-boundary tests."""
...
RUNBOOK = ROOT / "docs/experiments/suggest-moving-service-questions/v2-pilot/sequence-4-operator-runbook.md"
OPERATOR = SCRIPTS / "run_v2_sequence_4_live_preflight_operator.zsh"


def test_candidate_and_fixed_paths() -> None:
    candidate = (ROOT / "docs/experiments/suggest-moving-service-questions/v2-pilot/authorization-review/phase-candidates/sequence-4/inactive-sequence-4-preflight-authorization-candidate.toml").read_text()
    manifest = (ROOT / "docs/experiments/suggest-moving-service-questions/v2-pilot/authorization-review/phase-candidates/sequence-4/sequence-4-candidate-manifest.json").read_text()
    assert "sequence = 4" in candidate and '"sequence": 4' in manifest
    assert "004-storage_unknown" in manifest
...
def test_candidate_loaders_reject_cross_sequence_path_substitution(monkeypatch) -> None:
    import pytest
    import v2_sequence_3_authorization_candidate as sequence_3_candidate
    import v2_sequence_4_authorization_candidate as sequence_4_candidate
    from v2_phase_authorization_candidates import V2PhaseCandidateError
    monkeypatch.setattr(sequence_4_candidate, "CANDIDATE_PATH", sequence_3_candidate.CANDIDATE_PATH)
    with pytest.raises(V2PhaseCandidateError):
        sequence_4_candidate.load_sequence_4_preflight_candidate()
    monkeypatch.undo()
    monkeypatch.setattr(sequence_3_candidate, "CANDIDATE_PATH", sequence_4_candidate.CANDIDATE_PATH)
    with pytest.raises(V2PhaseCandidateError):
        sequence_3_candidate.load_sequence_3_preflight_candidate()


def test_sequence_4_public_tools_are_fixed_to_004_paths() -> None:
    for path in SCRIPTS.glob("*sequence_4*"):
        if path.is_file() and not path.name.startswith("test_") and path.suffix in {".py", ".sh", ".zsh"}:
            text = path.read_text()
            assert "003-storage_unknown" not in text
            assert "sequence=3" not in text and "expected_sequence=3" not in text


def test_sequence_4_operational_paths_do_not_collide_with_history() -> None:
    from v2_preflight_authorization_activation import activation_paths
    previous = {getattr(activation_paths(sequence=sequence), field) for sequence in (1, 2, 3)
                for field in ("installed", "installation", "review", "active", "activation", "journal", "closure")}
    current = activation_paths(sequence=4)
    for field in ("installed", "installation", "review", "active", "activation", "journal", "closure"):
        path = getattr(current, field)
        assert "004-storage_unknown" in path.name
        assert path not in previous
...
def _stubbed_operator(*, launcher_status: int = 0, signal_name: str | None = None,
    ...
    with tempfile.TemporaryDirectory(prefix="gotime-sequence4-shell-") as directory:
        root = Path(directory)
        script_path = root / "scripts/experiments/suggest_moving_service_questions/run_v2_sequence_4_live_preflight_operator.zsh"
        ...
        environment.update({"PATH": f"{bin_path}:{environment['PATH']}", "GOTIME_V2_SEQUENCE_4_OFFLINE_TEST": "1",
                            "GOTIME_V2_SEQUENCE_4_SYNTHETIC_INPUT_FD": str(read_fd),
                            "GOTIME_SEQUENCE4_CALL_LOG": str(call_log)})
...
def test_same_shell_success_does_not_disclose_credential() -> None:
    ...
    assert calls.count("run_openai_stage_b_v2_sequence_4_preflight_docker.sh") == 1
    assert calls.count("close_v2_sequence_4_preflight_authorization_docker.sh --reason success") == 1
...
def test_exact_public_sequence_4_synthetic_rehearsal() -> None:
    if shutil.which("docker") is None or shutil.which("zsh") is None:
        return
    with tempfile.TemporaryDirectory(prefix="gotime-v2-sequence4-rehearsal-") as directory:
        ...
        rendered_path = Path("/tmp") / f"gotime-sequence4-rehearsal-{os.getpid()}.toml"
        rendered_path.unlink(missing_ok=True)
        try:
            rendered = _public(root, "render_v2_sequence_4_preflight_authorization_candidate_docker.sh",
            ...
            installed = _public(root, "install_v2_sequence_4_preflight_authorization_for_review_docker.sh",
                "--source", str(rendered_path), "--expected-sha256", rendered["sha256"])
            reviewed = _public(root, "review_v2_sequence_4_preflight_authorization_activation_docker.sh",
            ...
            plan = _public(root, "plan_v2_sequence_4_preflight_authorization_activation_docker.sh",
            ...
            assert "004-storage_unknown" in plan["future_active_destination"] and plan["writes_performed"] == "false"
            activated_result = _public(root, "activate_v2_sequence_4_preflight_authorization_docker.sh",
                "--artifact-sha256", installed["sha256"], "--installation-record-sha256", installed["installation_record_sha256"],
            ...
            environment = os.environ.copy()
            environment.update({"GOTIME_V2_SEQUENCE_4_OFFLINE_TEST": "1", "GOTIME_V2_SEQUENCE_4_SYNTHETIC_INPUT_FD": str(read_fd)})
            result = subprocess.run(["zsh", "scripts/experiments/suggest_moving_service_questions/run_v2_sequence_4_live_preflight_operator.zsh"],
            ...
            assert (run_state / "004-storage_unknown-preflight.json").is_file()
            assert (run_state / "004-storage_unknown-preflight-evidence.json").is_file()
            evidence_path = run_state / "004-storage_unknown-preflight-evidence.json"
            assert (run_state / "004-storage_unknown-preflight-closure.json").is_file()
            assert not (run_state / "004-storage_unknown-preflight-authorization.toml").exists()
            ...
            evidence_review = _public(
                root, "review_v2_sequence_4_preflight_evidence_docker.sh",
                "--evidence-sha256", __import__("hashlib").sha256(evidence_path.read_bytes()).hexdigest(),
                "--input-tokens", str(evidence["input_tokens"]),
                "--conservative-cost", evidence["conservative_maximum_generation_cost"],
                "--reviewer", "Synthetic Evidence Reviewer", "--decision", "approve",
                "--reviewed-at", datetime.now(timezone.utc).replace(microsecond=0).strftime("%Y-%m-%dT%H:%M:%SZ"),
                "--token-count-plausible", "true", "--cost-within-limit", "true",
                "--frozen-bindings-confirmed", "true", "--evidence-history-confirmed", "true",
                "--notes", "Immediate synthetic evidence approval")
            assert evidence_review["generation_gate_binding_eligible"] == "true"
            assert (run_state / "004-storage_unknown-preflight-review.json").is_file()
            second = subprocess.run(["zsh", "scripts/experiments/suggest_moving_service_questions/run_v2_sequence_4_live_preflight_operator.zsh"],
...
if __name__ == "__main__":
    ...
    test_exact_public_sequence_4_synthetic_rehearsal()
    print("sequence_4_single_shell_tests=passed")
