"""Fixed loader boundary for the inactive sequence-4 preflight candidate."""
...
from v2_sequence_3_authorization_candidate import CANDIDATE_PATH as SEQUENCE_3_CANDIDATE_PATH

SEQUENCE = 4
AUDIT_PREFIX = "004-storage_unknown"
SEQUENCE_3_CANDIDATE_DIGEST = "4b6c2f3f3881a1979f7db628b6225b52a2aa89e155099dd386f4c95d8c048aed"
...
PACKAGE_ROOT = REPOSITORY_ROOT / (
    ...
    "phase-candidates/sequence-4"
)
CANDIDATE_PATH = PACKAGE_ROOT / "inactive-sequence-4-preflight-authorization-candidate.toml"
MANIFEST_PATH = PACKAGE_ROOT / "sequence-4-candidate-manifest.json"
...
def _regular_bytes(path: Path) -> bytes:
    if path.is_symlink() or not path.is_file():
        raise V2PhaseCandidateError("Sequence-4 candidate package path is unsafe.")
    return path.read_bytes()


def load_sequence_4_preflight_candidate(phase: str = "preflight") -> VerifiedPhaseCandidate:
    if phase != "preflight":
        raise V2PhaseCandidateError("Sequence-4 package supports preflight only.")
    ...
    expected_keys = {
        ...
        "sequence", "audit_prefix", "sequence_4_candidate_path", "sequence_4_candidate_digest",
        "sequence_3_candidate_path", "sequence_3_candidate_digest", "sequence_3_role",
    ...
    if set(manifest) != expected_keys:
        raise V2PhaseCandidateError("Sequence-4 candidate manifest shape drifted.")
    required = {
        "capability": "suggest_moving_service_questions",
        "manifest_version": "moving-service-openai-v2-preflight-sequence-4-candidate-manifest-v1",
        "purpose": "inactive_sequence_4_preflight_authorization_review",
        "package_status": "inactive_non_authoritative",
        "sequence": SEQUENCE,
        "audit_prefix": AUDIT_PREFIX,
        "sequence_4_candidate_path": str(CANDIDATE_PATH.relative_to(REPOSITORY_ROOT)),
        "sequence_4_candidate_digest": candidate_digest,
        "sequence_3_candidate_digest": SEQUENCE_3_CANDIDATE_DIGEST,
    ...
    if any(manifest.get(key) != value for key, value in required.items()):
        raise V2PhaseCandidateError("Sequence-4 candidate manifest drifted.")
    if (
        _digest(REPOSITORY_ROOT / manifest["sequence_3_candidate_path"])
        != SEQUENCE_3_CANDIDATE_DIGEST
        or (REPOSITORY_ROOT / manifest["sequence_3_candidate_path"]).resolve()
        != SEQUENCE_3_CANDIDATE_PATH.resolve()
        or _digest(REPOSITORY_ROOT / manifest["sequence_2_candidate_path"])
    ...
    for key, value in _expected_manifest_bindings().items():
        if manifest.get(key) != value:
            raise V2PhaseCandidateError("Sequence-4 frozen binding drifted.")
    if any(manifest.get(key) is not False for key in (
        "active_repository_authority", "valid_for_execution", "credential_access_authorized",
        "token_preflight_authorized", "ai_generation_authorized", "formal_evaluation_authorized",
        "stage_c_authorized", "production_use_authorized",
    )):
        raise V2PhaseCandidateError("Sequence-4 manifest grants authority.")
    artifact = tomllib.loads(candidate_bytes.decode("utf-8"))
    if (
        artifact.get("metadata", {}).get("candidate_version")
        != "moving-service-openai-v2-preflight-sequence-4-candidate-v1"
        or artifact.get("scope", {}).get("sequence") != SEQUENCE
    ):
        raise V2PhaseCandidateError("Sequence-4 candidate identity drifted.")
    ...
    if any(manifest[key] != artifact["bindings"][binding] for key, binding in manifest_to_binding.items()):
        raise V2PhaseCandidateError("Sequence-4 manifest and candidate bindings differ.")
