"""Dependency-free zsh process-boundary tests for frozen-v3 preflight."""
...
OPERATOR = ROOT / "scripts/experiments/suggest_moving_service_questions/run_v3_live_preflight_operator.zsh"
INTENT = "AUTHORIZE_ONE_STORAGE_UNKNOWN_V3_PREFLIGHT_ONLY"
...
def _run(*, launcher_status=0, signal_name=None, closure_status=0):
    ...
    with tempfile.TemporaryDirectory(prefix="gotime-v3-preflight-shell-") as directory:
        ...
        action = f"kill -{signal_name} $PPID; exit 99" if signal_name else ('test "$GOTIME_MOVING_SERVICE_EVAL_OPENAI_API_KEY" = synthetic-v3-secret || exit 88; test "$GOTIME_MOVING_SERVICE_EVAL_ENABLED" = 1 || exit 88; ' + f'test "$GOTIME_MOVING_SERVICE_EVAL_OPERATOR_INTENT" = {INTENT} || exit 88; exit {launcher_status}')
...
def test_success_failure_signals_and_closure_failure() -> None:
    result, calls = _run(); assert result.returncode == 0; assert "synthetic-v3-secret" not in result.stdout + result.stderr; assert "--reason success" in calls
    ...
    for signal, code in (("INT", 130), ("TERM", 143), ("HUP", 129)):
        result, calls = _run(signal_name=signal); assert result.returncode == code; assert "bounded_failure" in calls; assert "synthetic-v3-secret" not in result.stdout + result.stderr
