"""Internal fixed sequence-3 CLI used only by capability-specific wrappers."""
...
from v2_sequence_3_authorization_candidate import load_sequence_3_preflight_candidate
from v2_sequence_3_preflight_authorization import (
    activate_sequence_3_preflight,
    install_sequence_3_for_review,
    plan_sequence_3_activation,
    recover_sequence_3_preflight,
    review_sequence_3_activation,
)
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if args.operation == "render":
            ...
            result = render_preflight_candidate_for_sequence(sequence=3,
                candidate_loader=load_sequence_3_preflight_candidate, output_path=output,
            ...
            result = install_sequence_3_for_review(source=Path(args.source), expected_sha256=args.expected_sha256, now=now)
            _emit({key: result[key] for key in ("installed_path", "sha256", "installation_record", "installation_record_sha256", "authoritative")})
        elif args.operation == "review":
            result = review_sequence_3_activation(artifact_sha256=args.artifact_sha256,
            ...
            _emit(plan_sequence_3_activation(artifact_sha256=args.artifact_sha256,
            ...
            _emit(activate_sequence_3_preflight(artifact_sha256=args.artifact_sha256,
                installation_record_sha256=args.installation_record_sha256,
            ...
            result = recover_sequence_3_preflight(reason=args.reason, now=now)
            path = activation_paths(sequence=3).closure
            _emit({"closure_path": path.resolve(), "closure_sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
                   ...
                   "sequence": 3, "generation_authorized": False})
        else:
            active = load_active_preflight_authorization(now=now, expected_sequence=3)
            remaining = int((active.authorization.expires_at - now).total_seconds())
            if remaining < 180:
                raise ValueError("fewer than 180 seconds remain")
            _emit({"sequence": 3, "phase": "preflight", "transaction_state": "committed",
                   "generation_authorized": False, "seconds_remaining": remaining})
    except (OSError, ValueError) as error:
        print(f"sequence_3_{args.operation.replace('-', '_')}_error: rejected", file=sys.stderr)
        return 4
    return 0
