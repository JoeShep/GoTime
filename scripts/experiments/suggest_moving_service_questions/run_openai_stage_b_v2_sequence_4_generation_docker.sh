...
network_args=""
[ "${GOTIME_V2_SEQUENCE_4_GENERATION_OFFLINE_TEST:-}" = "1" ] && network_args="--network none"
...
exec docker run --rm $network_args --user "$(id -u):$(id -g)" --read-only --tmpfs /tmp:rw,nosuid,nodev,noexec,size=64m \
  ...
  --env GOTIME_V2_SEQUENCE_4_GENERATION_OFFLINE_TEST \
  --env GOTIME_V2_SEQUENCE_4_GENERATION_SYNTHETIC_SCENARIO \
