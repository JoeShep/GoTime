...
recover_if_required() {
  ...
  if [[ "$launcher_started" == true && "$closure_complete" != true ]]; then
    sh scripts/experiments/suggest_moving_service_questions/close_v2_sequence_4_preflight_authorization_docker.sh \
...
sh scripts/experiments/suggest_moving_service_questions/verify_v2_sequence_4_preflight_authorization_docker.sh

if [[ "${GOTIME_V2_SEQUENCE_4_OFFLINE_TEST:-}" == "1" && -n "${GOTIME_V2_SEQUENCE_4_SYNTHETIC_INPUT_FD:-}" ]]; then
  read -r -u "$GOTIME_V2_SEQUENCE_4_SYNTHETIC_INPUT_FD" GOTIME_MOVING_SERVICE_EVAL_OPENAI_API_KEY
else
...
sh scripts/experiments/suggest_moving_service_questions/run_openai_stage_b_v2_sequence_4_preflight_docker.sh
sh scripts/experiments/suggest_moving_service_questions/close_v2_sequence_4_preflight_authorization_docker.sh --reason success
