"""Static boundaries for the fixed frozen-v4 generation workflow."""
...
PUBLIC = (
    "render_v4_sequence_4_generation_authorization_candidate_docker.sh",
    "install_v4_sequence_4_generation_authorization_for_review_docker.sh",
    "review_v4_sequence_4_generation_authorization_activation_docker.sh",
    "plan_v4_sequence_4_generation_authorization_activation_docker.sh",
    "activate_v4_sequence_4_generation_authorization_docker.sh",
    "verify_v4_sequence_4_generation_authorization_docker.sh",
    "run_openai_stage_b_v4_sequence_4_generation_docker.sh",
    "run_v4_sequence_4_live_generation_operator.zsh",
    "review_v4_sequence_4_generation_response_docker.sh",
    "delete_v4_sequence_4_generation_response_evidence_docker.sh",
    "close_v4_sequence_4_generation_authorization_docker.sh",
)
...
def test_same_shell_credential_boundary_and_traps() -> None:
    text = (SCRIPTS / "run_v4_sequence_4_live_generation_operator.zsh").read_text()
    assert 'read -s "GOTIME_MOVING_SERVICE_EVAL_OPENAI_API_KEY?OpenAI evaluation key: "' in text
    assert "AUTHORIZE_ONE_STORAGE_UNKNOWN_V4_GENERATION_ONLY" in text
...
def test_live_generation_has_no_preflight_and_credential_is_not_an_argument() -> None:
    runner = (SCRIPTS / "run_openai_stage_b_v4_sequence_4_generation_live.py").read_text()
    launcher = (SCRIPTS / "run_openai_stage_b_v4_sequence_4_generation_docker.sh").read_text()
...
def test_rehearsal_covers_all_public_commands_and_five_scenarios() -> None:
    rehearsal = (SCRIPTS / "rehearse_v4_sequence_4_generation_workflow.sh").read_text()
    for name in PUBLIC:
    ...
    assert "GOTIME_V4_SEQUENCE_4_GENERATION_SYNTHETIC_INPUT_FD" in rehearsal


def test_every_runbook_script_exists_and_is_executable() -> None:
    runbook = (ROOT / "docs/experiments/suggest-moving-service-questions/v4-generation-operator-runbook.md").read_text()
...
def _stubbed_operator(*, launcher_status: int = 0, signal_name: str | None = None,
    ...
    operator = SCRIPTS / "run_v4_sequence_4_live_generation_operator.zsh"
    with tempfile.TemporaryDirectory(prefix="gotime-v4-sequence4-shell-") as directory:
        root = Path(directory)
        ...
        action = (
            ...
            'test "$GOTIME_MOVING_SERVICE_EVAL_OPERATOR_INTENT" = AUTHORIZE_ONE_STORAGE_UNKNOWN_V4_GENERATION_ONLY || exit 88; '
        ...
        environment.update({
            ...
            "GOTIME_V4_SEQUENCE_4_GENERATION_OFFLINE_TEST": "1",
            "GOTIME_V4_SEQUENCE_4_GENERATION_SYNTHETIC_INPUT_FD": str(read_fd),
            "GOTIME_V4_CALL_LOG": str(call_log),
        })
...
def test_same_shell_success_and_failure_preserve_boundary_and_exit_codes() -> None:
    ...
    assert success_calls.count("run_openai_stage_b_v4_sequence_4_generation_docker.sh") == 1
    assert "close_v4_sequence_4_generation_authorization_docker.sh --reason success" in success_calls
