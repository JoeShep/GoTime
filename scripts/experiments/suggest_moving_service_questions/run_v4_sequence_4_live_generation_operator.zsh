...
cleanup_generation_environment() {
  ...
  if [[ "$launcher_started" == true && "$closure_complete" != true ]]; then
    sh scripts/experiments/suggest_moving_service_questions/close_v4_sequence_4_generation_authorization_docker.sh --reason bounded_failure >/dev/null 2>&1 || true
  fi
...
if [[ "${GOTIME_V4_SEQUENCE_4_GENERATION_OFFLINE_TEST:-}" == "1" && -n "${GOTIME_V4_SEQUENCE_4_GENERATION_SYNTHETIC_INPUT_FD:-}" ]]; then
  read -r -u "$GOTIME_V4_SEQUENCE_4_GENERATION_SYNTHETIC_INPUT_FD" GOTIME_MOVING_SERVICE_EVAL_OPENAI_API_KEY
else
...
export GOTIME_MOVING_SERVICE_EVAL_OPERATOR_INTENT=AUTHORIZE_ONE_STORAGE_UNKNOWN_V4_GENERATION_ONLY
launcher_started=true
sh scripts/experiments/suggest_moving_service_questions/run_openai_stage_b_v4_sequence_4_generation_docker.sh
sh scripts/experiments/suggest_moving_service_questions/close_v4_sequence_4_generation_authorization_docker.sh --reason success
