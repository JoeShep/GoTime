"""Fixed public-command backend for frozen-v3 sequence-1 preflight."""
...
from v3_sequence_1_preflight import (
    DEFAULT_OUTPUT_ROOT, OPERATOR_INTENT, RENDERED_TMP, V3PreflightError,
...
def current() -> datetime:
    if os.environ.get("GOTIME_V3_SEQUENCE_1_PREFLIGHT_OFFLINE_TEST") == "1":
        synthetic = os.environ.get("GOTIME_V3_SEQUENCE_1_PREFLIGHT_SYNTHETIC_NOW")
        if synthetic:
...
def main(argv=None) -> int:
    ...
    try:
        if args.operation == "render": emit(render(approver=args.approver, approved_at=args.approved_at, activated_at=args.activated_at, expires_at=args.expires_at, reason=args.reason, now=now))
            ...
            if any(path.exists() for path in vars(target).values() if path not in (target.execution, target.closed)) or RENDERED_TMP.exists(): raise V3PreflightError("real v3 preflight state exists")
            emit({**status, "run_series_id": "moving-service-stage-b-v3-pilot-20260807", "sequence": 1, "generation_authorized": False, "readiness_valid": True})
    except (OSError, ValueError, AssertionError):
        print(f"v3_sequence_1_{args.operation.replace('-', '_')}_error=rejected", file=sys.stderr); return 4
    return 0
