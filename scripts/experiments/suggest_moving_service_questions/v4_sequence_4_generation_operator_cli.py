"""Fixed synthetic-only v4 generation CLI; public wrappers provide isolation."""
...
from v4_sequence_1_preflight import (
    OPERATOR_INTENT as PREFLIGHT_OPERATOR_INTENT,
    PROVIDER_FINGERPRINT as PREFLIGHT_PROVIDER_FINGERPRINT,
    RENDERED_TMP as PREFLIGHT_RENDERED_TMP,
    activate as activate_preflight,
    activation_review as review_preflight_activation,
    execute_preflight,
    install as install_preflight,
...
from v4_sequence_4_generation_gate import (
    ...
    RUN_SERIES_ID, activate_generation_authority, active_generation_manifest_bytes,
    ...
    verify_live_generation_precredential, verify_resolved_generation_candidate,
    write_generation_outcome,
)
from v4_sequence_4_generation_rehearsal_assertions import assert_rehearsal_scenario
...
def now() -> datetime:
    if os.environ.get("GOTIME_V4_SEQUENCE_4_GENERATION_OFFLINE_TEST") == "1":
        synthetic = os.environ.get("GOTIME_V4_SEQUENCE_4_GENERATION_SYNTHETIC_NOW")
        if synthetic:
...
def verify_readiness():
    ...
    if any(path.exists() for path in vars(paths).values()):
        raise ValueError("real v4 generation state exists")
    return {**result, "generation_authority": False, "readiness_valid": True}


def prepare_realistic_synthetic_preflight() -> None:
    """Create the real sequence-1 lifecycle shapes for offline generation rehearsal."""
    clock = now().replace(microsecond=0)
    target = preflight_paths(OUTPUT_ROOT, REPOSITORY_ROOT)
    if any(path.exists() for path in vars(target).values()
           if path not in {target.closed, target.execution}):
        raise ValueError("synthetic preflight state is not empty")
    PREFLIGHT_RENDERED_TMP.unlink(missing_ok=True)
    rendered = render_preflight(
        approver="Synthetic Preflight Approver",
        approved_at=(clock - timedelta(seconds=12)).isoformat().replace("+00:00", "Z"),
        activated_at=(clock - timedelta(seconds=10)).isoformat().replace("+00:00", "Z"),
        expires_at=(clock + timedelta(seconds=890)).isoformat().replace("+00:00", "Z"),
        reason="Synthetic realistic completed preflight lifecycle",
        now=clock - timedelta(seconds=8),
    )
    installed = install_preflight(
        source=PREFLIGHT_RENDERED_TMP, expected_sha256=rendered["sha256"],
        now=clock - timedelta(seconds=8), output_root=OUTPUT_ROOT)
    reviewed = review_preflight_activation(
        artifact_sha256=rendered["sha256"], reviewer="Synthetic Preflight Reviewer",
        decision="approve",
        reviewed_at=(clock - timedelta(seconds=7)).isoformat().replace("+00:00", "Z"),
        notes="Synthetic preflight activation review",
        now=clock - timedelta(seconds=7), output_root=OUTPUT_ROOT)
    activate_preflight(
        artifact_sha256=rendered["sha256"],
        installation_sha256=installed["installation_record_digest"],
        review_sha256=reviewed["review_sha256"], operator="Synthetic Preflight Operator",
        operator_intent=PREFLIGHT_OPERATOR_INTENT, now=clock - timedelta(seconds=5),
    ...
    class Transport:
        ...
        def request_fingerprint(self, _request): return PREFLIGHT_PROVIDER_FINGERPRINT
        def preflight(self, request):
            if request is not self.prepared.provider_request:
                raise AssertionError("prepared preflight request was rebuilt")
            return OpenAIPreflightResult(
                PREFLIGHT_PROVIDER_FINGERPRINT, 4242, 1.25, Decimal("0.0024242"))

    execute_preflight(environment={
        "GOTIME_MOVING_SERVICE_EVAL_ENABLED": "1",
        "GOTIME_MOVING_SERVICE_EVAL_OPERATOR_INTENT": PREFLIGHT_OPERATOR_INTENT,
        "GOTIME_MOVING_SERVICE_EVAL_OPENAI_API_KEY": "synthetic-not-a-real-credential",
    }, now=clock, client_builder=lambda _credential: Owned(),
        transport_factory=lambda _client, prepared: Transport(prepared),
        output_root=OUTPUT_ROOT, repository_root=REPOSITORY_ROOT)
    evidence = json.loads(target.evidence.read_text())
    evidence_digest = digest(target.evidence)
    ...
    review_preflight_evidence(
        ...
        notes="Synthetic approved preflight evidence", now=datetime.fromisoformat(
            reviewed_at.replace("Z", "+00:00")), output_root=OUTPUT_ROOT,
        repository_root=REPOSITORY_ROOT)
    PREFLIGHT_RENDERED_TMP.unlink(missing_ok=True)
    emit({"realistic_completed_preflight_history": True,
          "historical_preflight_closure_valid": True,
          "synthetic_input_tokens": 4242,
          "synthetic_conservative_cost": "0.0024242"})


def assert_live_entry_boundaries() -> None:
    """Exercise the actual live credential boundary against active synthetic state."""
    from run_openai_stage_b_v4_sequence_4_generation_live import (
    ...
    protected = (paths.review_rendered, paths.installation, paths.activation_review,
                 paths.active, paths.activation, paths.transaction, execution)
    ...
    verified, _ = verify_attempt_then_read_credential(environment=positive, now=now())
    expected_active = active_generation_manifest_bytes(str(verified.authorization_digest))
    if execution.read_bytes() != expected_active or execution.read_bytes() == closed.read_bytes():
        raise AssertionError("positive active generation state is not exact")

    artifact = tomllib.loads(paths.active.read_text())
    artifact["bindings"]["deterministic_request_digest"] = "0" * 64
    lines = []
    for section, values in artifact.items():
        lines.append(f"[{section}]")
        for key, value in values.items():
            lines.append(f"{key} = {json.dumps(value)}" if not isinstance(value, bool)
                         else f"{key} = {str(value).lower()}")
        lines.append("")
    mutated_bytes = ("\n".join(lines).rstrip() + "\n").encode()
    paths.active.write_bytes(mutated_bytes); paths.review_rendered.write_bytes(mutated_bytes)
    authorization_digest = digest(paths.active)
    installation = json.loads(paths.installation.read_text())
    installation["installed_digest"] = authorization_digest
    paths.installation.write_text(json.dumps(installation, indent=2, sort_keys=True) + "\n")
    review = json.loads(paths.activation_review.read_text())
    review["installed_artifact_digest"] = authorization_digest
    review["installation_record_digest"] = digest(paths.installation)
    paths.activation_review.write_text(json.dumps(review, indent=2, sort_keys=True) + "\n")
    ...
    activation["installation_record_digest"] = digest(paths.installation)
    activation["activation_review_digest"] = digest(paths.activation_review)
    activation["active_manifest_digest"] = digest(execution)
    activation["transaction_id"] = hashlib.sha256(
        f"{authorization_digest}:{activation['activated_at']}".encode()).hexdigest()[:32]
    paths.activation.write_text(json.dumps(activation, indent=2, sort_keys=True) + "\n")
    transaction = json.loads(paths.transaction.read_text())
    transaction["transaction_id"] = activation["transaction_id"]
    transaction["artifact_digest"] = authorization_digest
    transaction["activation_record_digest"] = digest(paths.activation)
    paths.transaction.write_text(json.dumps(transaction, indent=2, sort_keys=True) + "\n")
    wrong_authorization = environment()
    try:
        verify_attempt_then_read_credential(environment=wrong_authorization, now=now())
    except ValueError:
        pass
    else:
        raise AssertionError("wrong active authorization reached credential lookup")

    for path, content in original.items(): path.write_bytes(content)
    execution.write_bytes(closed.read_bytes())
    wrong_manifest = environment()
    try:
        verify_attempt_then_read_credential(environment=wrong_manifest, now=now())
    except ValueError:
        pass
    else:
        raise AssertionError("wrong active manifest reached credential lookup")
    finally:
        for path, content in original.items(): path.write_bytes(content)

    if positive.credential_lookups != 1 or wrong_authorization.credential_lookups or wrong_manifest.credential_lookups:
        raise AssertionError("live credential-boundary counts drifted")
    emit({
        "live_entry_positive_verified": True,
        "realistic_completed_preflight_history": True,
        "current_manifest_active_generation": True,
        "historical_preflight_closure_valid": True,
        "wrong_active_authorization_rejected_precredential": True,
        "wrong_active_manifest_rejected_precredential": True,
        "credential_lookup_attempts_positive": positive.credential_lookups,
        "credential_lookup_attempts_wrong_authorization": wrong_authorization.credential_lookups,
        "credential_lookup_attempts_wrong_manifest": wrong_manifest.credential_lookups,
        "client_constructions_wrong_authorization": 0,
        "client_constructions_wrong_manifest": 0,
        "provider_generations_wrong_authorization": 0,
        "provider_generations_wrong_manifest": 0,
    })
...
def render(args) -> None:
    ...
    artifact = {
        "metadata": {"capability": "suggest_moving_service_questions", "authorization_version": "moving-service-openai-v4-generation-sequence-4-v1", "authorization_status": "approved_v4_generation", "phase": "generation", "active_repository_authority": True},
        "bindings": candidate["bindings"], "required_v4_preflight": candidate["required_v4_preflight"],
...
def cleanup_expired_review(args) -> None:
    paths = state_paths()
    rendered = Path("/tmp/gotime-v4-sequence-4-generation-authorization.toml")
...
def verify_active() -> None:
    verified = verify_live_generation_precredential(
        output_root=OUTPUT_ROOT, repository_root=REPOSITORY_ROOT, now=now())
...
def parser() -> argparse.ArgumentParser:
    ...
    commands.add_parser("prepare-realistic-synthetic-preflight")
    commands.add_parser("assert-live-entry-boundaries")
...
def main(argv=None) -> int:
    ...
    try:
        ...
        elif args.operation == "prepare-realistic-synthetic-preflight": prepare_realistic_synthetic_preflight()
        elif args.operation == "assert-live-entry-boundaries": assert_live_entry_boundaries()
