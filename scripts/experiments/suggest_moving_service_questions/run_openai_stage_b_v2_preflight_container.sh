...
cli=scripts/experiments/suggest_moving_service_questions/run_openai_stage_b_v2_preflight_live.py
[ -f "$cli" ] || exit 6
...
if [ "${GOTIME_V2_TWO_GATE_OFFLINE_TEST:-}" = "1" ]; then
  exec python scripts/experiments/suggest_moving_service_questions/run_openai_stage_b_v2_preflight_synthetic.py
fi
