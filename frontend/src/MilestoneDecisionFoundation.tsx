...
function timing(milestone: Milestone) {
  const earliest = milestone.target_earliest_date
  const latest = milestone.target_latest_date
  if (!earliest) return 'Target timing not set'
  if (!latest) return `Target: on or after ${earliest}`
  if (earliest === latest) return `Target: ${earliest}`
  return `Target window: ${earliest} to ${latest}`
}
...
export function MilestoneDecisionFoundation({ plan, onPlanUpdated }: Props) {
  ...
  async function perform(action: () => Promise<RelocationPlan>) {
    setSaving(true)
    setError(null)
  ...
  async function saveMilestone() {
    ...
    const saved = await perform(() => milestoneId
  ...
  async function saveDecision() {
    ...
    const saved = await perform(() => decisionId
  ...
  return (
    <section className="plan-foundation mb-4" aria-labelledby="plan-foundation-heading">
      ...
      {milestoneDraft && <Card className="mb-3"><Card.Body>
        <Card.Title>{milestoneId ? 'Edit milestone' : 'Add milestone'}</Card.Title>
        <Form onSubmit={(event) => { event.preventDefault(); void saveMilestone() }}>
          <Row className="g-3">
            ...
            <Col sm={6}><Form.Group controlId="milestone-latest"><Form.Label>Latest target date <span className="text-muted">(optional)</span></Form.Label><Form.Control type="date" value={milestoneDraft.target_latest_date ?? ''} onChange={(event) => setMilestoneDraft({ ...milestoneDraft, target_latest_date: event.target.value || null })} /><Form.Text>Leave blank for an open-ended “on or after” target.</Form.Text></Form.Group></Col>
          </Row>
          <Stack direction="horizontal" gap={2} className="mt-3"><Button type="submit" disabled={saving}>Save milestone</Button><Button variant="outline-secondary" onClick={() => setMilestoneDraft(null)}>Cancel</Button></Stack>
        </Form>
      </Card.Body></Card>}

      {decisionDraft && <Card className="mb-3"><Card.Body>
        <Card.Title>{decisionId ? 'Edit decision' : 'Add decision'}</Card.Title>
        <Form onSubmit={(event) => { event.preventDefault(); void saveDecision() }}>
          ...
          <Stack direction="horizontal" gap={2} className="mt-3"><Button type="submit" disabled={saving}>Save decision</Button><Button variant="outline-secondary" onClick={() => setDecisionDraft(null)}>Cancel</Button></Stack>
        </Form>
      </Card.Body></Card>}

      {plan.milestones.length === 0 && !milestoneDraft && <p className="text-muted">No milestones yet.</p>}
      <Row className="g-3">{plan.milestones.map((milestone) => <Col lg={6} key={milestone.id}><Card className="h-100"><Card.Body>
        <div className="d-flex justify-content-between gap-2"><Card.Title as="h4">{milestone.title}</Card.Title><Badge bg={milestone.status === 'achieved' ? 'success' : 'secondary'}>{milestone.status === 'achieved' ? 'Achieved' : 'Pending'}</Badge></div>
        <p className="text-muted">{timing(milestone)}</p>{milestone.description && <p>{milestone.description}</p>}
        <Stack direction="horizontal" gap={2} className="flex-wrap"><Button size="sm" variant="outline-secondary" onClick={() => { setMilestoneId(milestone.id); setMilestoneDraft({ title: milestone.title, description: milestone.description, target_earliest_date: milestone.target_earliest_date, target_latest_date: milestone.target_latest_date }) }}>Edit</Button><Button size="sm" variant={milestone.status === 'achieved' ? 'outline-secondary' : 'success'} disabled={saving} onClick={() => void perform(() => changeMilestoneAchievement(milestone.id, milestone.status !== 'achieved'))}>{milestone.status === 'achieved' ? 'Return to pending' : 'Mark achieved'}</Button></Stack>
        {plan.decisions.filter((decision) => decision.milestone_id === milestone.id).map((decision) => <Card className="mt-3" key={decision.id}><Card.Body><div className="d-flex justify-content-between gap-2"><Card.Title as="h5">{decision.title}</Card.Title><Badge bg={decision.status === 'resolved' ? 'success' : 'warning'} text={decision.status === 'resolved' ? undefined : 'dark'}>{decision.status === 'resolved' ? 'Resolved' : 'Unresolved'}</Badge></div>{decision.description && <p>{decision.description}</p>}<Form.Group><Form.Label>Select an option</Form.Label><Form.Select aria-label={`Selection for ${decision.title}`} disabled={saving} value={decision.selected_option_id ?? ''} onChange={(event) => void perform(() => changeDecisionSelection(decision.id, event.target.value || null))}><option value="">Unresolved</option>{decision.options.map((option) => <option key={option.id} value={option.id}>{option.title}</option>)}</Form.Select></Form.Group><Button className="mt-2" size="sm" variant="outline-secondary" onClick={() => { setDecisionId(decision.id); setDecisionDraft({ title: decision.title, description: decision.description, milestone_id: decision.milestone_id, options: decision.options }) }}>Edit decision</Button></Card.Body></Card>)}
